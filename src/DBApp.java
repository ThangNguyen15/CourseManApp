...
//    }
  ...
  public String select(String sql) {
     ...
     *
     *  <p><pre>If succeeded
     *    return a <tt>String</tt> representation of the result set; or
     *    "empty" if the result set is empty
     *  else
     *    display an error message
     *    return null</pre>
   ...
   *  executes the SELECT statement <tt>sql</tt>. 
   * 
   *  <p><pre>if there are data rows matching the query
   *    return a <tt>Vector<Vector></tt>, each element of which is a Vector
   *    of data values in a row
   *  else
   *    if an error occured
   *        display an error message
   *    return null
   *  </pre>
   */
  public Vector<Vector> selectData(String sql) {
    Statement s = null;
    ...
    try {
      ...
      // debug
      System.out.printf("%d columns%n",cols);
   ...
   * @effects 
   *   if sql is valid and results in an non-empty set of records returned from the database
   *    return true
   *   else
   *    return false
